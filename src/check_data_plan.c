...
/*   Updated: 2018/04/10 15:52:37 by prossi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_data_plan_td(t_stuff *e, char *str)
{
	int		i;
	char	*nb;
	...
	if (!(nb = ft_strsub(str, 1, ft_strlen(str) - 1)))
		return (-1);
	if (check_data_plan_fh(e, nb) == -1)
		return (-1);
	ft_strdel(&nb);
	return (0);
}

int		check_data_plan_sd(t_stuff *e, char *str)
{
	int		i;
	int		nbequal;
	int		nbdot;
