...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   check_data_cylindre.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: prossi <prossi@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/11/24 14:31:38 by prossi            #+#    #+#             */
/*   Updated: 2018/04/23 12:36:49 by prossi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_data_cylindre_td(t_stuff *e, char *str)
{
	int		i;
	char	*nb;
	...
	if (!(nb = ft_strsub(str, 1, ft_strlen(str) - 1)))
		return (-1);
	if (check_data_cylindre_fh(e, nb) == -1)
		return (-1);
	ft_strdel(&nb);
	return (0);
}

int		check_data_cylindre_sd(t_stuff *e, char *str)
{
	int		i;
	int		nbequal;
	int		nbdot;
...
int		check_data_cylindre(t_stuff *e, int y)
{
	...
	while (++tmp < y + 13)
	{
		...
		if (!(str = ft_strsub(e->b.tab[tmp], e->d.equal + 1, \
			...
			return (-1);
		if (check_data_cylindre_td(e, str) == -1)
			return (-1);
		i++;
		ft_strdel(&str);
	}
