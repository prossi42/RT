/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   terminal_end_aff_new_sphere_sd.c                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: prossi <prossi@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/04/17 13:09:53 by prossi            #+#    #+#             */
/*   Updated: 2018/04/17 13:15:10 by prossi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

void	end_aff_new_sphere_seh(t_stuff *e, char *tmp)
{
	e->i.term.tab[e->i.term.indextab] = \
	ft_strcpy(e->i.term.tab[e->i.term.indextab], tmp);
	e->i.term.tabfill = 1;
	ft_strclr(e->i.term.wbuf);
	ft_strdel(&tmp);
	if (e->i.nobj.first == 9)
	{
		e->i.nobj.first = -1;
		multi_thread(e);
		apercu_courant(e);
	}
	else
	{
		e->i.nobj.first++;
		terminal(e);
	}
}

char	*end_aff_new_sphere_sh(t_stuff *e, char *tmp)
{
	if (e->i.nobj.first == 9)
	{
		tmp = ft_strcpy(tmp, "Reflex ? : ");
		tmp = ft_strcat(tmp, e->i.term.wbuf);
		e->sph->reflex = ft_myatod(e->i.term.wbuf);
		if (e->sph->reflex > 1)
			e->sph->reflex = 1;
		if (e->sph->reflex < 0)
			e->sph->reflex = 0;
	}
	return (tmp);
}
